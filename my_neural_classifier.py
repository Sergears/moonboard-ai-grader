"""
architecture: n layers of relu, output layer of 17 sigmoids
...
def compute_cost(AL, Y, cost_func, lambd, parameters):
    """
    Arguments:
    AL -- probability vector corresponding to your label predictions, shape (17, number of examples)
    Y -- true "label" vector (containing 0 if not the grade, 1 if the grade), shape (17, number of examples)
...
def compute_n_correct(AL, Y):
    ...
    for i in range(m):
        y = Y[:,i]
        a = AL[:,i]
        predicted_grade = np.argmax(a)
        correct_grade = np.argmax(y)
        n_correct += int(predicted_grade == correct_grade)
        error_sum += np.abs(predicted_grade - correct_grade)
    mae = error_sum / m  # mean squared error
...
def L_model_backward(AL, Y, caches, cost_func, act, last_act):
    ...
    Y -- true "label" vector (containing 0 if not the grade, 1 if the grade)
    caches -- list of caches containing:
...
def train_model(X, Y, X_test, Y_test, layers_dims, mini_batch_size, eta=0.1, n_epochs=30, cost_func = 'cross_entropy', act='sigmoid', last_act='sigmoid', w_init=1.0, lambd=0):
    ...
    Y -- true "label" vector (containing 0 if not the actual grade, 1 if actual grade), of shape (17, number of examples)
    ...
    for i in range(0, n_epochs):
        ...
        if i % 10 == 0:
            ...
            print('epoch:', i, ', cost:', "{:.3f}".format(cost), 'training preformance:', training_result, 'test preformance:', test_result)
